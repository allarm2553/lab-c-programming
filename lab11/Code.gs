/**
 * Web App for Lab 11: C to Microcontroller & Embedded Systems Bridge
...
function doGet(e) {
  return HtmlService.createTemplateFromFile('index')
    .evaluate()
    .setTitle('Lab 11: การเชื่อมโยงภาษา C สู่ไมโครคอนโทรลเลอร์ (C to Microcontroller & Embedded Bridge)')
...
function submitLabData(data) {
  try {
    ...
    var sheetName = "Lab C 11 Submissions";
    ...
    var folderName = "Lab C 11 Attachments";
    ...
    var codeKeywords = ["millis|currentMs|time","setup|loop|task|Task","if|else","printf|Serial","struct|float|bool"];
    var q1Keywords = ["delay","millis","blocking","ค้าง","เรียลไทม์|multitask|task"];
    var q2Keywords = ["setup","loop","main","เริ่มต้น","วนซ้ำ|ตลอดเวลา|super-loop"];
    ...
    var isB1Correct = ["currentMillis","millis()"].indexOf(b1) !== -1;
    if (isB1Correct) blankCorrectCount++;
    

    var b2 = (data.blank2 || "").toString().trim();
    var isB2Correct = ["interval","500"].indexOf(b2) !== -1;
