...
<body>
  ...
  function __formatString(formatStr, ...args) {
    ...
    rendered = rendered.replace(/%(\\d+)?(\\.\\d+)?([a-zA-Z%]+)/g, (match, width, precision, spec) => {
      ...
      if (spec === 'd' || spec === 'i' || spec === 'u' || spec === 'zu' || spec === 'ld' || spec === 'lu' || spec === 'hhu' || spec === 'hu') {
  ...
  cleanCode = cleanCode.replace(/typedef\\s+struct\\s*(?:[a-zA-Z0-9_]+)?\\s*\\{([^}]+)\\}\\s*([a-zA-Z0-9_]+)\\s*;/g, (match, body, aliasName) => {
    ...
    memberLines.forEach(line => {
      ...
      if (!trimmed) return;
      const namePart = trimmed.replace(/^(?:const\\s+|unsigned\\s+|signed\\s+)?(?:int|char|float|double|short|long|void)\\s*(?:\\*)*/g, '').trim();
  ...
  cleanCode = cleanCode.replace(/\\b(?:void|int|float|double|char|bool)\\s+([a-zA-Z0-9_]+)\\s*\\(([^)]*)\\)\\s*\\{/g, (match, funcName, params) => {
    if (funcName === 'main') return 'function main() {';
    const cleanParams = params.split(',').map(p => {
      let trimmed = p.trim();
      if (!trimmed || trimmed === 'void') return '';
      return trimmed.replace(/^(?:const\\s+|unsigned\\s+|signed\\s+)?(?:struct\\s+[a-zA-Z0-9_]+\\s*\\*?|[a-zA-Z0-9_]+\\s*\\*?)\\s*/, '').replace(/^\\*/, '').trim();
    }).filter(p => p.length > 0).join(', ');
    return 'function ' + funcName + '(' + cleanParams + ') {';
  });

  // 4.4 Transform struct variable instantiations: struct Point p1 = {10, 20}; OR Student s1 = {"Somchai", 95};
  ...
  // 4.6 Transform strcpy(dest, src) -> dest = src
  ...
  cleanCode = cleanCode.replace(/scanf\\s*\\(\\s*([^)]+)\\s*\\)/g, (m, argsStr) => {
    ...
    const setters = args.slice(1).map(target => {
      ...
      if (/^[a-zA-Z_$][a-zA-Z0-9_$.]*$/.test(varName)) {
        return '((_v) => { try { ' + varName + ' = _v; } catch(e) {} })';
      }
      return '((_v) => {})';
    });
  ...
  // 8.5 Transform address-of & in function calls
  cleanCode = cleanCode.replace(/&([a-zA-Z0-9_]+)/g, '$1');

  // 9. Transform for loops
  cleanCode = cleanCode.replace(/for\\s*\\(\\s*(?:const\\s+|unsigned\\s+|signed\\s+)?(?:int|float|double|char|long|short)\\s+/g, 'for (let ');

  // 10. Transform primitive variable declarations
  cleanCode = cleanCode.replace(/(?:const\\s+|static\\s+|volatile\\s+)?\\b(?:unsigned\\s+|signed\\s+)?(?:int|char|short|long|float|double|void)(?:\\s+(?:int|long|short))*\\s+([a-zA-Z0-9_,\\s=\\[\\]\\*\\-\\+\\/\\.\\(\\)]+);/g, (m, declList) => {
    ...
    const items = declList.split(',').map(item => {
      const trimmed = item.trim();
      if (trimmed.includes('=')) {
        return trimmed.replace(/^\\*([a-zA-Z0-9_]+)/, '$1');
      }
    ...
    return 'let ' + items.join(', ') + ';';
  });

  // 10.4 Transform pointer dereference assignments: *ptr = 200; -> ptr = 200;
  cleanCode = cleanCode.replace(/\\*([a-zA-Z0-9_]+)\\s*=/g, '$1 =');

  // 11. Transform printf calls
  ...
  try {
    const fn = new Function('__printf', '__scanf', 'fopen', 'fclose', 'fprintf', 'fputs', 'fputc', 'fgets', 'fscanf', 'strlen', 'strcpy', 'strcmp', 'strcat', 'sizeof', 'Math', wrappedCode);
    const ret = fn(__printf, __scanf, fopen, fclose, fprintf, fputs, fputc, fgets, fscanf, strlen, strcpy, strcmp, strcat, sizeof, Math);
