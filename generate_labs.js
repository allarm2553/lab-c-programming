...
<body>
  ...
  // Robust variable declaration parsing: handles modulo %, bitwise &, |, ^, ~, <, >, ternary ?: etc.
  cleanCode = cleanCode.replace(/(?:const\\s+|static\\s+|volatile\\s+)?\\b(?:unsigned\\s+|signed\\s+)?(?:int|char|short|long|float|double|void|bool)(?:\\s+(?:int|long|short))*\\s+([^;{}]+);/g, (m, declList) => {
