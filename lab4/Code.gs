/**
 * Web App for Lab 4: Loops & Nested Loops
...
function doGet(e) {
  return HtmlService.createTemplateFromFile('index')
    .evaluate()
    .setTitle('Lab 4: โครงสร้างควบคุมแบบวนซ้ำ (Loops)')
...
function submitLabData(data) {
  try {
    ...
    var sheetName = "Lab C 4 Submissions";
    ...
    var folderName = "Lab C 4 Attachments";
    ...
    var codeKeywords = ["for","while","scanf","rows","n","\\*"];
    ...
    var isB1Correct = ["count <= 3","count<=3","count < 4","count<4"].indexOf(b1) !== -1;
    if (isB1Correct) blankCorrectCount++;
    

    var b2 = (data.blank2 || "").toString().trim();
    var isB2Correct = ["count++","count ++","++count","count += 1","count=count+1","count = count + 1"].indexOf(b2) !== -1;
