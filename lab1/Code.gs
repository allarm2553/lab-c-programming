/**
 * Web App for Lab 1: Variables, Data Types & Basic I/O
...
function doGet(e) {
  return HtmlService.createTemplateFromFile('index')
    .evaluate()
    .setTitle('Lab 1: ตัวแปร ชนิดข้อมูล และการรับส่งข้อมูลพื้นฐาน')
...
function submitLabData(data) {
  try {
    ...
    var sheetName = "Lab C 1 Submissions";
...
ระบบอนุญาตให้ส่งได้เพียง 1 ครั้งเท่านั้น (หากต้องการส่งใหม่ กรุณาติดต่ออาจารย์ผู้สอน)"
    ...
    var folderName = "Lab C 1 Attachments";
    ...
    var isB1Correct = ["&num","& num"].indexOf(b1) !== -1;
    if (isB1Correct) blankCorrectCount++;
    

    var b2 = (data.blank2 || "").toString().trim();
    var isB2Correct = ["%f"].indexOf(b2) !== -1;
