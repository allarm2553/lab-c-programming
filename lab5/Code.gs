...
function submitLabData(data) {
  try {
    ...
    var codeKeywords = ["float|void|double","return","scanf","printf","&|\\*"];
    ...
    var isB1Correct = ["exp == 0","exp==0","exp <= 0","exp<=0"].indexOf(b1) !== -1;
    if (isB1Correct) blankCorrectCount++;
    

    var b2 = (data.blank2 || "").toString().trim();
    var isB2Correct = ["power(base, exp - 1)","power(base,exp-1)","power(base, exp-1)"].indexOf(b2) !== -1;
